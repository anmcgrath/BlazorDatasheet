...
public class VirtualMergesLayerTests
{
    ...
    private static IRenderedComponent<Datasheet> RenderSheet(TestContext context, Sheet sheet)
    {
    ...
    public async Task Setting_A_Value_With_No_Merges_Does_Not_Render_The_Merges_Layer()
    {
        ...
        // the pane still hands the layer its parameters again, but the layer doesn't ask for a render itself
        RenderCount(cut).Should().Be(renders + 1);
    }
