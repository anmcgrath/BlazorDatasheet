...
{
	public static SheetModel FromSheet(Sheet sheet, List<CellFormat> formats, Action<string>? onWarning = null)
	{
		...
		var conditionalFormats = sheet.ConditionalFormats.GetAllFormats().ToList();

		// Warn once per ignored rule type rather than once per region it is applied to.
		foreach (var ignoredType in conditionalFormats.Select(x => x.Data.GetType())
			         .Where(IsIgnoredForSerialization)
			         .Distinct())
			onWarning?.Invoke(
				$"The conditional format {ignoredType.Name} on sheet {sheet.Name} was not written because it is marked with [{nameof(SerializationIgnoreAttribute)}].");

		sheetModel.ConditionalFormats = conditionalFormats
			.Where(x => !IsIgnoredForSerialization(x.Data.GetType()))
	...
	private static bool IsIgnoredForSerialization(Type type) =>
		type.IsDefined(typeof(SerializationIgnoreAttribute), inherit: true);
