...
public class RowHeadingLayoutProvider : IGridLayoutProvider
{
    ...
    public double TotalHeight => _sheet.Rows.GetVisualHeightBetween(ViewRegion.Top, ViewRegion.Bottom + 1);
    public int NumRows => _sheet.NumRows;
    public int NumColumns => 1;
    public double ComputeLeftPosition(int col) => 0;

    public double ComputeTopPosition(int row) =>
        _sheet.Rows.GetVisualTop(row) - _sheet.Rows.GetVisualTop(ViewRegion.Top);
