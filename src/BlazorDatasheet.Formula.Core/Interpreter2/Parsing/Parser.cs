...
using Lexer = BlazorDatasheet.Formula.Core.Interpreter2.Lexing.Lexer;
...
public class Parser
{
    ...
    private Expression ParseRangeExpression()
    {
        ...
        if (rangeParts.Count == 2)
            return new ReferenceExpression(new RangeReference(rangeParts[0], rangeParts[1]));

        return new ReferenceExpression(new MultiReference(rangeParts.ToArray()));
    }
